import { PrismaClient, Token, UserToken } from '@prisma/client';
...
export class TokenResolutionService {
  ...
  async resolveToken(
    ...
    const globalToken = await this.tokenService.getToken(chain, normalizedAddress);
  ...
  private async addNewToken(
    ...
    // Try Alchemy first
    try {
      const metadata = await this.alchemyService.getTokenMetadata(chain, address);
    ...
    // Try contract call
    try {
      const contractData = await this.fetchTokenFromContract(chain, address);
      ...
      console.warn(`Contract call failed for ${address} on ${chain}:`, error);
    }
  ...
  private async fetchTokenFromContract(
    ...
    try {
      ...
      console.error(`Failed to fetch token metadata from contract ${address} on ${chain}:`, error);
  ...
  async removeUserToken(
    ...
    const poolCount = await this.prisma.pool.count({
      where: {
        ownerId: userId,
        OR: [
          { token0Address: { in: [await this.getTokenAddress(tokenId)] } },
          { token1Address: { in: [await this.getTokenAddress(tokenId)] } }
        ]
      }
    });
