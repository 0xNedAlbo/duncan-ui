...
import { TOKEN_ADDRESSES, INVALID_ADDRESSES, mockTokens } from '../../__tests__/fixtures/tokens';
...
describe('TokenService', () => {
  ...
  describe('findOrCreateToken', () => {
    it('should create new token when it does not exist', async () => {
      const token = await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      
      expect(token.chain).toBe('ethereum');
      expect(token.address).toBe(TOKEN_ADDRESSES.ethereum.WETH.toLowerCase());
    ...
    it('should return existing token when it already exists', async () => {
      // Create token first
      const firstToken = await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      
      // Call again - should return same token
      const secondToken = await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
    ...
    it('should normalize address to lowercase', async () => {
      const upperCaseAddress = TOKEN_ADDRESSES.ethereum.WETH.toUpperCase();
      const token = await service.findOrCreateToken('ethereum', upperCaseAddress);
    ...
    it('should handle Alchemy API errors gracefully', async () => {
      ...
      const token = await service.findOrCreateToken('ethereum', nonExistentToken);
    ...
    it('should throw error for invalid chain', async () => {
      await expect(
        service.findOrCreateToken('invalid-chain', TOKEN_ADDRESSES.ethereum.WETH)
  ...
  describe('getToken', () => {
    it('should return token if it exists', async () => {
      // Create token first
      const createdToken = await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      
      // Get token
      const foundToken = await service.getToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
    ...
    it('should return null if token does not exist', async () => {
      const token = await service.getToken('ethereum', TOKEN_ADDRESSES.ethereum.USDC);
      expect(token).toBeNull();
    });

    it('should handle case-insensitive address lookup', async () => {
      // Create with lowercase
      await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      
      // Find with uppercase
      const token = await service.getToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH.toUpperCase());
  ...
  describe('searchTokens', () => {
    ...
    it('should search tokens by symbol', async () => {
      ...
      expect(tokens.every(token => token.symbol.includes('WETH'))).toBe(true);
    });

    it('should search tokens by name', async () => {
      const tokens = await service.searchTokens({ query: 'Wrapped' });
      expect(tokens.length).toBeGreaterThanOrEqual(1);
      expect(tokens.every(token => token.name.includes('Wrapped'))).toBe(true);
    });

    it('should filter by chain', async () => {
      const ethereumTokens = await service.searchTokens({ chain: 'ethereum' });
      expect(ethereumTokens.every(token => token.chain === 'ethereum')).toBe(true);
    });

    it('should filter verified tokens only', async () => {
      const verifiedTokens = await service.searchTokens({ verifiedOnly: true });
      expect(verifiedTokens.every(token => token.verified === true)).toBe(true);
    });

    it('should apply limit and offset', async () => {
      const firstPage = await service.searchTokens({ limit: 1, offset: 0 });
      const secondPage = await service.searchTokens({ limit: 1, offset: 1 });
    ...
    it('should prioritize verified tokens', async () => {
      ...
      const firstUnverifiedIndex = tokens.findIndex(token => !token.verified);
      const lastVerifiedIndex = tokens.reduce((lastIndex, token, index) => 
  ...
  describe('getTokensByAddresses', () => {
    it('should return tokens for given addresses', async () => {
      // Create tokens
      await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.USDC);

      const addresses = [TOKEN_ADDRESSES.ethereum.WETH, TOKEN_ADDRESSES.ethereum.USDC];
      const tokens = await service.getTokensByAddresses('ethereum', addresses);
    ...
    it('should filter by chain correctly', async () => {
      ...
      await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      await service.findOrCreateToken('arbitrum', TOKEN_ADDRESSES.arbitrum.WETH);

      const ethereumTokens = await service.getTokensByAddresses('ethereum', [TOKEN_ADDRESSES.ethereum.WETH]);
  ...
  describe('createTokensFromAddresses', () => {
    it('should create new tokens and return all tokens', async () => {
      const addresses = [TOKEN_ADDRESSES.ethereum.WETH, TOKEN_ADDRESSES.ethereum.USDC];
      const tokens = await service.createTokensFromAddresses('ethereum', addresses);
      
      expect(tokens).toHaveLength(2);
      expect(tokens.every(token => token.chain === 'ethereum')).toBe(true);
    });

    it('should reuse existing tokens and create only new ones', async () => {
      // Create one token first
      const existingToken = await service.findOrCreateToken('ethereum', TOKEN_ADDRESSES.ethereum.WETH);
      ...
      const wethToken = tokens.find(t => t.address === TOKEN_ADDRESSES.ethereum.WETH.toLowerCase());
      expect(wethToken!.id).toBe(existingToken.id);
    });

    it('should return empty array for empty addresses', async () => {
      const tokens = await service.createTokensFromAddresses('ethereum', []);
      expect(tokens).toEqual([]);
    });
  });
  ...
  describe('validation', () => {
    ...
    it('should validate supported chains', async () => {
      await expect(
        service.getToken('unsupported-chain', TOKEN_ADDRESSES.ethereum.WETH)
      ).rejects.toThrow('Unsupported chain');
    });
  });
});