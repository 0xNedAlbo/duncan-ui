...
export class TokenService {
  ...
  async findOrCreateToken(chain: string, address: string): Promise<Token> {
    ...
    try {
      const metadata = await this.alchemyService.getTokenMetadata(chain, normalizedAddress);
      ...
      console.warn(`Failed to fetch token metadata from Alchemy for ${address} on ${chain}:`, error);
   ...
   */
  async getTokensByAddresses(chain: string, addresses: string[]): Promise<Token[]> {
   ...
   */
  async createTokensFromAddresses(chain: string, addresses: string[]): Promise<Token[]> {
    ...
    const existingTokens = await this.getTokensByAddresses(chain, normalizedAddresses);
    const existingAddresses = new Set(existingTokens.map(token => token.address));
    ...
    try {
      const metadataList = await this.alchemyService.getTokenMetadataBatchLarge(chain, newAddresses);
   ...
   */
  isMetadataStale(token: Token, maxAgeMs: number = 7 * 24 * 60 * 60 * 1000): boolean {
  ...
  async refreshStaleTokens(chain?: string): Promise<void> {
    ...
    for (const token of staleTokens) {
      try {
        ...
        console.warn(`Failed to refresh token ${token.address} on ${token.chain}:`, error);
      }
    }
  }
  ...
  private validateInput(chain: string, address: string): void {
    ...
    if (!this.alchemyService.isChainSupported(chain)) {
      throw new Error(`Unsupported chain: ${chain}. Supported chains: ${this.alchemyService.getSupportedChains().join(', ')}`);
    }
  }
