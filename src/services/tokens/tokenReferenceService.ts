...
export class TokenReferenceService {
  ...
  async findOrCreateReference(
    ...
    const tokenInfo = await this.tokenResolver.resolveToken(chain, address, userId);

    if (tokenInfo.isGlobal) {
      // Create reference to global token
      return await this.createGlobalTokenReference(tokenInfo.id, chain, address, tokenInfo.symbol);
    } else {
      // Create reference to user token
      return await this.createUserTokenReference(tokenInfo.id, chain, address, tokenInfo.symbol);
    }
  }
   ...
   */
  async getReferenceById(referenceId: string): Promise<TokenReferenceWithToken | null> {
   ...
   */
  async findReference(chain: string, address: string): Promise<TokenReferenceWithToken | null> {
  ...
  getUnifiedTokenData(reference: TokenReferenceWithToken): UnifiedTokenData {
    ...
    throw new Error('Invalid token reference: neither global nor user token is set');
  }
  ...
  isGlobalReference(reference: TokenReferenceWithToken): boolean {
    return reference.tokenType === 'global' && reference.globalToken !== null;
  }
   ...
   */
  getTokenFromReference(reference: TokenReferenceWithToken): Token | UserToken {
   ...
   */
  async updateReferenceCache(referenceId: string, symbol: string): Promise<void> {
