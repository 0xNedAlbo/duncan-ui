...
import { TokenResolutionService, TokenInfo } from '../tokens/tokenResolutionService';
import {
...
export class PoolService {
  ...
  /**
   * Find or create a pool with automatic token resolution
   */
  async findOrCreatePool(
    ...
    if (!poolExists) {
      throw new Error(`Pool does not exist on-chain for ${token0Info.symbol}/${token1Info.symbol} with ${fee/100}% fee`);
    }
    ...
    const pool = await this.prisma.pool.create({
      data: {
        ...
        ownerId: (!token0Info.isGlobal || !token1Info.isGlobal) ? userId : null,
      },
      include: {
        token0: true,
        token1: true
  ...
  /**
   * Get pool by ID with resolved token information
   */
  async getPoolById(poolId: string, userId?: string): Promise<PoolWithTokens | null> {
    ...
    if (userId) {
      const [token0Info, token1Info] = await Promise.all([
        this.tokenResolver.resolveToken(pool.chain, pool.token0Address, userId),
        this.tokenResolver.resolveToken(pool.chain, pool.token1Address, userId)
      ]);
   ...
   */
  async searchPools(options: PoolSearchOptions = {}): Promise<PoolWithTokens[]> {
    const {
      chain,
      token0,
      token1,
    ...
    if (token0 || token1) {
      ...
      if (token0Normalized && token1Normalized) {
        const [sortedToken0, sortedToken1] = sortTokens(token0Normalized, token1Normalized);
  ...
  async updatePoolState(poolId: string): Promise<void> {
    ...
    try {
      const poolState = await this.fetchPoolState(pool.chain, pool.poolAddress);
      
      // Get token decimals for price calculation
      const [token0Info, token1Info] = await Promise.all([
        this.tokenResolver.resolveToken(pool.chain, pool.token0Address, ''), // Empty userId for global lookup
        this.tokenResolver.resolveToken(pool.chain, pool.token1Address, '')
      ]);
      ...
      console.error(`Failed to update pool state for pool ${poolId}:`, error);
      throw error;
    }
  }
   ...
   * Verify pool exists on-chain
   */
  private async verifyPoolExists(chain: string, poolAddress: string): Promise<boolean> {
    const chainConfig = CHAIN_CONFIG[chain as keyof typeof CHAIN_CONFIG];
    if (!chainConfig) {
      return false;
    }
   ...
   */
  private async fetchPoolState(chain: string, poolAddress: string): Promise<PoolState> {
    const chainConfig = CHAIN_CONFIG[chain as keyof typeof CHAIN_CONFIG];
    if (!chainConfig) {
      throw new Error(`Unsupported chain: ${chain}`);
    }
