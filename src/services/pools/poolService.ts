...
function sortTokens(tokenA: string, tokenB: string): [string, string] {
    const sorted = sortTokensUtil({ address: tokenA }, { address: tokenB });
...
export class PoolService {
    ...
    private tokenReferences: TokenReferenceService;

    constructor(
        requiredClients: Pick<Clients, 'prisma'>,
        requiredServices: Pick<Services, 'tokenResolutionService' | 'tokenReferenceService'>
    ) {
        this.prisma = requiredClients.prisma;
        this.tokenResolver = requiredServices.tokenResolutionService;
        this.tokenReferences = requiredServices.tokenReferenceService;
    }
    ...
    async findOrCreatePool(
        ...
        const [token0Ref, token1Ref] = await Promise.all([
            this.tokenReferences.findOrCreateReference(
                chain,
                sortedToken0,
                userId
            ),
        ...
        if (existingPool) {
            ...
            if (!updatedPool) {
                throw new Error(
                    `Pool ${existingPool.id} not found after update`
                );
            }

            return {
                ...updatedPool,
                token0Data: this.tokenReferences.getUnifiedTokenData(
                    updatedPool.token0Ref
                ),
                token1Data: this.tokenReferences.getUnifiedTokenData(
        ...
        if (!poolExists) {
            const token0Data =
                this.tokenReferences.getUnifiedTokenData(token0Ref);
            const token1Data =
                this.tokenReferences.getUnifiedTokenData(token1Ref);
            throw new Error(
        ...
        const poolOwnerId =
            this.tokenReferences.isUserReference(token0Ref) ||
            this.tokenReferences.isUserReference(token1Ref)
                ? userId
                : null;
    ...
    async disconnect(): Promise<void> {
        ...
        await this.tokenReferences.disconnect();
