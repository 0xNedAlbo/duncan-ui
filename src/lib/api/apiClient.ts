...
export class ApiClient {
  ...
  private shouldSkipAuth(url: string): boolean {
    return this.config.skipAuthForRoutes.some(route => url.startsWith(route));
  }
  ...
  async request<T = any>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        console.log(`[API] ${requestOptions.method || 'GET'} ${url}${attempt > 0 ? ` (attempt ${attempt + 1})` : ''}`);
        ...
        if (!interceptedResponse.ok) {
          const apiError = await ApiError.fromResponse(interceptedResponse);
          
          // Don't retry auth errors or client errors (4xx except rate limiting)
          if (!apiError.isRetryable() || attempt === retries) {
            const finalError = await this.applyErrorInterceptors(apiError, url, requestOptions);
            throw finalError;
          }
          ...
          if (attempt < retries) {
            ...
            console.log(`[API] Retrying in ${delay}ms due to:`, apiError.message);
        ...
        console.log(`[API] ✓ ${requestOptions.method || 'GET'} ${url} (${interceptedResponse.status})`);
        ...
        if (!apiError.isRetryable() || attempt === retries) {
          const finalError = await this.applyErrorInterceptors(apiError, url, requestOptions);
          console.error(`[API] ✗ ${requestOptions.method || 'GET'} ${url}:`, finalError.message);
          throw finalError;
        }
    ...
    throw lastError || new ApiError(ApiErrorCode.UNKNOWN_ERROR, 'Request failed after all retries');
  }
