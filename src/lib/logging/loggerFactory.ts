...
class LoggerRegistry {
    ...
    private shouldLogService(serviceName: string): boolean {
        ...
        return this.serviceFilter.some(
            (filter) =>
                lowerServiceName.includes(filter) ||
                filter.includes(lowerServiceName)
        );
    }
...
export const LogPatterns = {
    ...
    methodError: (
        ...
        logger.error(
            { method, error: error.message, stack: error.stack, ...context },
            `Error in ${method}`
        );
    },
