...
//
//   import {
//     Fraction, formatFractionHuman, formatFractionRangeHuman,
//     FORMAT_PRESET_DE, FORMAT_PRESET_EN
//   } from "./fraction-format";
//
//   const fr: Fraction<bigint> = { num: 1234n, den: 10n ** 10n };
//   console.log(formatFractionHuman(fr, FORMAT_PRESET_DE)); // "0,₍6₎1234…"
//
//   const a = { num: 123456n, den: 10n**12n };
//   const b = { num: 123999n, den: 10n**12n };
//   console.log(formatFractionRangeHuman(a, b, FORMAT_PRESET_EN)); // "0.₍4₎1234[56–99]…"
...
export function fractionToDecimalParts(
    ...
    for (let i = 0; i < maxFracDigits && r !== 0n; i++) {
        ...
        const digit = r / d; // 0..9
        r = r % d;
        // Append a single digit 0..9; toString() is safe and short
...
export function formatFractionHuman(
    ...
    if (intPart !== "0") {
        ...
        return (
            signStr + grouped + decimalSep + fracDigits + (truncated ? "…" : "")
        );
    }
...
export function formatFractionRangeHuman(
    ...
    // Same zero-run: compress it and surface differences
    const zeros = zA;
    ...
    const remA = tailA.slice(
        k,
        k + Math.max(0, o.mantissaDigits - common.length)
    );
    const remB = tailB.slice(
        k,
        k + Math.max(0, o.mantissaDigits - common.length)
    );
