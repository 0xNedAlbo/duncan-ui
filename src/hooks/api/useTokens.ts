...
export function useTokenSearch(
  ...
  options?: Omit<UseQueryOptions<TokenSearchResponse, ApiError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
    queryKey: QUERY_KEYS.tokenSearch(params),
    queryFn: () => apiClient.get<TokenSearchResponse>('/api/tokens/search', { params }),
...
export function useTokenBatch(
  ...
  options?: Omit<UseQueryOptions<TokenBatchResponse, ApiError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
    queryKey: QUERY_KEYS.tokenBatch(request.addresses, request.chain),
    queryFn: () => apiClient.post<TokenBatchResponse>('/api/tokens/batch', request),
...
export function useToken(
  address: string,
  chain: string,
  options?: Omit<UseQueryOptions<TokenDetailsResponse, ApiError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
    queryKey: QUERY_KEYS.tokenDetails(address, chain),
    queryFn: () => apiClient.get<TokenDetailsResponse>(`/api/tokens/${address}`, { 
      params: { chain } 
    }),
...
export function useInvalidateTokens() {
  ...
  return {
    ...
    invalidateAll: () => {
      return queryClient.invalidateQueries({ queryKey: QUERY_KEYS.tokens });
    },
    
    // Invalidate specific token
    invalidateToken: (address: string, chain: string) => {
      return queryClient.invalidateQueries({ queryKey: QUERY_KEYS.tokenDetails(address, chain) });
    },
    
    // Invalidate search results
    invalidateSearch: (params?: TokenSearchParams) => {
      if (params) {
        return queryClient.invalidateQueries({ queryKey: QUERY_KEYS.tokenSearch(params) });
      }
      return queryClient.invalidateQueries({ 
        predicate: (query) => query.queryKey[0] === 'tokens' && query.queryKey[1] === 'search'
      });
    },
  };
}
...
export function useTokenCacheInfo() {
  ...
  return {
    getCachedTokens: () => {
      return queryClient.getQueryCache().findAll({ queryKey: QUERY_KEYS.tokens });
    },
    ...
    getStats: () => {
      ...
      return {
        ...
        searchQueries: tokenQueries.filter(q => q.queryKey[1] === 'search').length,
        detailQueries: tokenQueries.filter(q => q.queryKey[1] === 'details').length,
        batchQueries: tokenQueries.filter(q => q.queryKey[1] === 'batch').length,
      };
    },
  };
}