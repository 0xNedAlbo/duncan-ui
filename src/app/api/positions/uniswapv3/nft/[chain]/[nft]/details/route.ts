...
/**
 * GET /api/positions/uniswapv3/nft/[chain]/[nft]/details - Get complete position details
 *
...
export const GET = withAuthAndLogging<PositionDetailsResponse>(
    async (
        ...
        try {
                ...
                (async () => {
                    try {
                        if (!curveDataService.validatePosition(position)) {
                            log.debug(
                                { positionId: position.id, status: position.status, liquidity: position.liquidity },
            ...
            logError(log, error, {
                endpoint: "positions/uniswapv3/nft/[chain]/[nft]/details",
                chain: params?.chain,
                nftId: params?.nft,
            });
