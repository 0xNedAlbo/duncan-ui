...
import { Copy, Plus, Key, MoreHorizontal, Trash2, ArrowLeft } from "lucide-react";
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-lg p-4 hover:border-slate-600 transition-colors">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className={`p-2 rounded-lg ${isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                        <Key size={20} />
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-4">
                            <div>
                                ...
                                <p className="text-sm text-slate-400 font-mono">ak_live_{apiKey.prefix}...</p>
                            </div>
                ...
                <div className="relative">
                    ...
                    {showDropdown && (
                        <div className="absolute right-0 top-full mt-1 w-32 bg-slate-800 border border-slate-700 rounded-lg shadow-lg z-10">
                            ...
                            >
                                <Trash2 size={14} />
                                {revokeApiKey.isPending ? t('apiKeys.actions.revoking') : t('apiKeys.actions.revoke')}
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md">
                    <div className="p-6">
                        <h2 className="text-xl font-semibold text-white mb-2">
                            {createdKey ? t('apiKeys.keyCreated') : t('apiKeys.create.title')}
                        </h2>
                        <p className="text-slate-400 mb-6">
                            {createdKey ? t('apiKeys.keyCreatedDescription') : t('apiKeys.create.description')}
                        </p>
                        ...
                        {createdKey ? (
                            <div className="space-y-4">
                                <div className="bg-slate-900/50 border border-slate-600 rounded-lg p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-sm font-medium text-white">{t('apiKeys.copyKey')}</span>
                                        <button
                                            onClick={handleCopy}
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex justify-end gap-3">
                                    ...
                                    >
                                        {createApiKey.isPending ? t('apiKeys.create.creating') : t('apiKeys.createKey')}
                                    </button>
                                </div>
                            </form>
                        )}
...
function ApiKeysContent() {
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center min-h-64">
                <div className="text-red-400">{t('apiKeys.errors.loadFailed')}</div>
            </div>
        );
    }
