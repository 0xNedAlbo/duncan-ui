 ...
 */
export function useIncreaseLiquidity(params: IncreaseLiquidityParams | null): UseIncreaseLiquidityResult {
  const [increaseError, setIncreaseError] = useState<Error | null>(null);
  const [addedLiquidity, setAddedLiquidity] = useState<bigint | undefined>(undefined);
  ...
  const increaseParams = params ? prepareIncreaseParams(params, slippageBps) : null;
    ...
    if (isSuccess && receipt) {
      ...
      const increaseLiquidityLog = receipt.logs.find(
        log =>
          ...
          log.topics[0] === '0x3067048beee31b25b2f1681f88dac838c8bba36af25bfb2b7cf7473a5847e35f' // IncreaseLiquidity event signature
      );
      ...
      if (increaseLiquidityLog && increaseLiquidityLog.data) {
        try {
          ...
          console.error('Failed to extract added liquidity from receipt:', error);
        }
      }
    }
  ...
  const increase = () => {
    if (!params || !increaseParams || !managerAddress) {
      setIncreaseError(new Error('Missing required parameters for increasing liquidity'));
      return;
    }
