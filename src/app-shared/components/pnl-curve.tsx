...
export function PnLCurve({
    ...
    const curveData = useMemo(() => {
        ...
        return data.map((point) => {
            const priceDisplay =
                Number(point.price) / Math.pow(10, params.baseTokenDecimals);
            const pnlDisplay =
                Number(point.pnl) / Math.pow(10, params.baseTokenDecimals); // Assuming quote token has same decimals

            return {
                price: priceDisplay,
                positionValue:
                    Number(point.positionValue) /
                    Math.pow(10, params.baseTokenDecimals),
                pnl: pnlDisplay,
        ...
        return (
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 shadow-xl backdrop-blur-sm">
                <p className="text-slate-300 text-sm">
                    <strong>{t("pnlCurve.price")}:</strong> $
                    {Number(label).toLocaleString()}
                </p>
                <p className="text-slate-300 text-sm">
                    <strong>{t("pnlCurve.positionValue")}:</strong> $
                ...
                >
                    <strong>{t("pnlCurve.pnl")}:</strong> $
                    {data.pnl.toLocaleString()} ({data.pnlPercent.toFixed(2)}%)
                </p>
